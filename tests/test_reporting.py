...
def test_reporter_writes_csv_and_html_for_a_completed_trial(tmp_path: Path) -> None:
    report = TrialReport(
        1, "trial-1", WorkerStatus.COMPLETED, ({
            "name": "throughput", "backend": "guidellm", "backend_version": "0.7.3",
            "raw_artifact": "results.json", "workloads": ({
                "index": 0, "configuration": {}, "metrics": {
    ...
    csv_path, html_path = Reporter(tmp_path).write(
        "output_tokens_per_second", (report,), (score,), None,
    )
