...
from vllm_optimizer.reporting.analysis import parameter_importance


def importance_section(ranking: tuple[TrialScore, ...]) -> str:
    if len(ranking) < 5:
        return (
            "<p class='muted'>Not shown: fewer than 5 eligible tuned trials. "
            "A percentage here would look precise without enough evidence.</p>"
        )
    values = parameter_importance(ranking)
    confidence = (
        "Exploratory association across evaluated trials, not causation. "
        "For each setting, vLLM Optimizer groups trial scores by observed value and "
