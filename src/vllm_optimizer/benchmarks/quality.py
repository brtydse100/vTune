...
def request_quality(metrics: object) -> dict[str, int | float]:
    ...
    return {"successful_requests": successful, "failed_requests": failed,
            "errored_requests": errored, "incomplete_requests": incomplete,
            "total_requests": total,
            "failure_percentage": 100 * failed / total if total else 100.0}


def aggregate_quality(workloads: object) -> dict[str, int | float]:
    summaries = [request_quality(item.get("metrics")) for item in workloads
                 if isinstance(item, Mapping)] if isinstance(workloads, (list, tuple)) else []
