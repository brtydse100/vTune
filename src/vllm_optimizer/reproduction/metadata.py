...
def _gpu_metadata() -> list[dict[str, str]]:
    ...
    if executable is None:
        return []
    output = _run(executable, "--query-gpu=name,uuid,driver_version,memory.total", "--format=csv,noheader,nounits")
    if not output:
        return []
    keys = ("name", "uuid", "driver_version", "memory_mib")
    return [dict(zip(keys, (part.strip() for part in line.split(",", 3)), strict=True)) for line in output.splitlines()]
