...
def validate_cli_options(args: Namespace) -> None:
    ...
    if action in (None, "validate"):
        ...
        _reject(failure_percentage is not None or accept_any, "failure policy options are only valid with reclassify")
        return
    ...
    if action in ("export", "reproduce"):
        ...
        _require(args.trial and len(args.trial) == 1, f"{action} requires exactly one --trial")
        ...
        _reject(failure_percentage is not None or accept_any, f"failure policy options cannot be used with {action}")
        return
    if action == "report":
        ...
        _reject(failure_percentage is not None or accept_any, "failure policy options cannot be used with report")
        return
    if action == "reclassify":
        ...
        _require(
            chosen != accept_any, "choose exactly one of --max-failure-percentage or --accept-any-request-failures"
        )
        if chosen and isinstance(failure_percentage, int | float):
            _require(0 <= failure_percentage <= 100, "--max-failure-percentage must be between 0 and 100")
        return
    ...
    _reject(failure_percentage is not None or accept_any, "failure policy options cannot be used with retry")
