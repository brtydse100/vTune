...
from vtune.measurement import benchmark_samples, sequentially_drifted
...
TrialRun = Callable[
    ...
    Awaitable[tuple[TrialReport, object, Mapping[str, float]]],
]


async def validate_drifted_finalists(
    directory: Path, session: RunAccumulator, results: RunResultsManager,
    run_id: str, started_at: str, metric: str, threshold: float,
    parameters_by_id: Mapping[str, TrialParameters],
    slots_by_id: Mapping[str, WorkerSlot | None],
    run_trial: TrialRun, warn: Callable[[str], None],
    source_run_id: str | None, sources: Mapping[str, Mapping[str, str]],
) -> None:
    ...
    for finalist in session.ranking[:2]:
        ...
        session.persist(results, run_id, metric, "running", started_at, None,
                        source_run_id, sources)


def _report_has_drift(report: TrialReport, metric: str, threshold: float) -> bool:
    return any(sequentially_drifted(values, threshold)
               for values in benchmark_samples(report.benchmarks, metric).values())
