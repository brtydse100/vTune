...
from vtune.reporting.analysis import parameter_effects, trial_metric
...
def history_chart(
    ...
    points = [(report.trial_id, scores[report.trial_id]) for report in trials
              if report.trial_id in scores]
    if not points:
    ...
    coords = [(40 + index * step, 280 - (value - low) / span * 230)
...
def comparison_chart(
    ...
    bars = "".join(
        ...
        f"<b>{value:.4f}</b></div>" for name, value in values
    )
...
def scatter_chart(trials: tuple[TrialReport, ...]) -> str:
    points = [(report.trial_id, throughput, latency) for report in trials
